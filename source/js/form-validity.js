...
const inputTextaria = document.getElementById('comment-field');
const inputSelect = document.getElementById('city-user');
const dropDownItems = document.querySelectorAll('.form-request__dropdown-item');
...
form.addEventListener('submit', async (event) => {
  ...
  const isTextariaValid = validateTextaria(inputTextaria);
  const isSelectValid = validateSelect(inputSelect);
  const isCheckboxValid = validateCheckbox(checkbox);

  if (isNameValid && isPhoneValid && isTextariaValid && isSelectValid && isCheckboxValid) {
    ...
    validateTextaria(inputTextaria);
...
function validateTextaria(inputElement) {
  clearError(inputElement);
...
inputTextaria.addEventListener('input', function() {
  validateTextaria(this);
